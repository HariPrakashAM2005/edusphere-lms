...
exports.googleCallback = exports.googleLogin = exports.verifyMfa = exports.setupMfa = exports.getMe = exports.resetPassword = exports.forgotPassword = exports.refreshToken = exports.logout = exports.login = exports.register = void 0;
...
const seedHashedPassword = bcryptjs_1.default.hashSync('Test@123', 10);
...
async function runWithFallback(dbAction, fallbackAction) {
    try {
...
const register = async (req, res) => {
    ...
    try {
        ...
        const { result, fallbackUsed } = await runWithFallback(async () => {
...
const login = async (req, res) => {
    ...
    try {
        const { result: user, fallbackUsed } = await runWithFallback(async () => {
        ...
        // Generate tokens
        const accessToken = jsonwebtoken_1.default.sign({ id: user.id, email: user.email, role: user.role }, JWT_SECRET, { expiresIn: TOKEN_EXPIRY });
        const refreshToken = jsonwebtoken_1.default.sign({ id: user.id, email: user.email }, JWT_REFRESH_SECRET, { expiresIn: REFRESH_TOKEN_EXPIRY });
        // Save refresh token
...
const refreshToken = async (req, res) => {
    ...
    try {
        ...
        const { result: user } = await runWithFallback(async () => {
...
const resetPassword = async (req, res) => {
    ...
    try {
        ...
        const { fallbackUsed } = await runWithFallback(async () => {
            const dbUser = await prisma.user.findUnique({ where: { email } });
            if (!dbUser)
                throw new Error('USER_NOT_FOUND');
...
const googleLogin = (req, res) => {
    ...
    const url = `https://accounts.google.com/o/oauth2/v2/auth?response_type=code&client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&scope=email%20profile`;
    res.redirect(url);
};
...
const googleCallback = async (req, res) => {
    ...
    if (!code) {
        const mockToken = jsonwebtoken_1.default.sign({ id: 'google-mock-id', email: 'google-user@test.com', role: 'STUDENT' }, JWT_SECRET, { expiresIn: TOKEN_EXPIRY });
