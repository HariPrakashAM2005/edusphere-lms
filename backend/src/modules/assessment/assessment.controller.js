...
exports.gradeAttempt = exports.getAssessmentAttempts = exports.publishResults = exports.getAssessmentAnalytics = exports.getResultsRank = exports.getResults = exports.submitAttempt = exports.startAttempt = exports.getAssessmentById = exports.getStudentUpcoming = exports.getAssessments = exports.createAssessment = void 0;
...
const seedMockAssessmentsIfEmpty = async (courseId, createdBy) => {
    try {
        ...
        if (!existing) {
            ...
            await (0, question_bank_service_1.generateRandomQuestions)(newAssessment.id, 6);
        }
    }
    catch (err) {
...
const createAssessment = async (req, res) => {
    ...
    try {
        ...
        const totalMarks = questionsList.reduce((sum, q) => sum + (parseFloat(q.marks) || 0), 0);
...
exports.getStudentUpcoming = getStudentUpcoming;
const getAssessmentById = async (req, res) => {
    ...
    try {
        ...
        const cleanQuestions = assessment.questions.map(q => {
            ...
            if (q.options) {
                optionsList = (0, question_bank_service_1.shuffleOptions)(q.options);
            }
            return {
                id: q.id,
                text: q.text,
...
const submitAttempt = async (req, res) => {
    ...
    try {
        ...
        const submissions = Array.isArray(answers) ? answers : [];
        ...
        for (const q of questions) {
            ...
            const graded = await (0, auto_grading_service_1.gradeSubmission)(q, studentAnsText);
            // Save answer in DB
        ...
        // Score computation
        const gradingStats = (0, auto_grading_service_1.calculateScore)(gradedAnswers.map(ans => ({
            marksAwarded: ans.marksAwarded || 0,
            isCorrect: !!ans.isCorrect,
            question: { type: ans.question.type }
        })), attempt.assessment.totalMarks, attempt.assessment.passingScore);
        // Update attempt
        ...
        try {
            ...
            (0, socket_1.emitAssessmentSubmitted)(attempt.assessmentId, studentName, attempt.assessment.title, attempt.id);
        }
        catch (broadcastErr) {
...
const getResultsRank = async (req, res) => {
    const assessmentId = req.params.id;
    try {
...
exports.getResultsRank = getResultsRank;
const getAssessmentAnalytics = async (req, res) => {
    ...
    try {
        ...
        for (const a of assessment.attempts) {
            const rep = await (0, proctoring_service_1.generateProctoringReport)(a.id);
...
exports.getAssessmentAnalytics = getAssessmentAnalytics;
    ...
    try {
        ...
        try {
            await (0, socket_1.sendRealTimeNotification)({
                ...
                message: `Your exam "${attempt.assessment.title}" has been graded. Score: ${totalScore}/${attempt.assessment.totalMarks} (${percentage}%). Feedback: ${answers.map((a) => a.feedback).filter(Boolean).join('. ') || 'Well done!'}`,
            });
        }
        catch (notifErr) {
...
exports.gradeAttempt = gradeAttempt;
