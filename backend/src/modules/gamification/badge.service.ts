 ...
 */
export const seedDefaultBadges = async (): Promise<void> => {
 ...
 */
export const checkAndAwardBadges = async (userId: string): Promise<any[]> => {
  ...
  for (const badge of badges) {
    ...
    switch (badge.name) {
      case 'First Steps': qualifies = lessonCount >= 1; break;
      case 'Double Digits': qualifies = lessonCount >= 10; break;
      case 'Halfway There': qualifies = lessonCount >= 50; break;
      case 'Century Club': qualifies = lessonCount >= 100; break;

      case 'Quiz Starter': qualifies = quizPassCount >= 1; break;
      case 'Decathlon': qualifies = quizPassCount >= 10; break;
      case 'Flawless Victory': qualifies = perfectQuizzes >= 1; break;
      case 'Academic Excellence': qualifies = highScores >= 5; break;
      case 'Knowledge Explorer': qualifies = uniqueQuizCourses >= 3; break;

      case 'Streak Starter': qualifies = loginStreak >= 7; break;
      case 'Streak Master': qualifies = loginStreak >= 30; break;
      case 'Streak Legend': qualifies = loginStreak >= 100; break;
      case 'Unstoppable': qualifies = assignmentStreak >= 5; break;

      case 'First Post': qualifies = discussionStartCount >= 1; break;
      case 'Social Butterfly': qualifies = discussionReplyCount >= 10; break;
      case 'Helpful Peer': qualifies = upvotesReceived >= 5; break;
      case 'Community Pillar': qualifies = upvotesReceived >= 20; break;

      case 'Early Bird': qualifies = earlySubmissions >= 1; break;
      case 'Speed Demon': qualifies = fastSubmissions >= 1; break;
      case 'Consistent Scholar': qualifies = attendanceStreak >= 15; break;
      
      case 'Level 5 Achiever': qualifies = currentLevel >= 5; break;
      case 'Level 10 Achiever': qualifies = currentLevel >= 10; break;
      case 'Level 25 Achiever': qualifies = currentLevel >= 25; break;

      case 'Night Owl': qualifies = nightStudyCount >= 1; break;
      case 'Weekend Warrior': qualifies = weekendStudyCount >= 3; break;
    }
    ...
    if (qualifies) {
      ...
      await awardXP(userId, `badge_earned_${badge.name.replace(/\s+/g, '_').toLowerCase()}`, 100, badge.id);
