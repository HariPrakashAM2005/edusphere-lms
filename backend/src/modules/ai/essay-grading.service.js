...
exports.gradeEssay = exports.calculatePlagiarismScore = void 0;
const axios_1 = __importDefault(require("axios"));
const OPENAI_API_KEY = process.env.OPENAI_API_KEY || '';
const OPENAI_API_URL = 'https://api.openai.com/v1/chat/completions';
// Helper: Calculate token Jaccard string similarity (Plagiarism Checker)
const calculatePlagiarismScore = (studentText, referenceText) => {
    if (!studentText || !referenceText)
        return 0;
    const clean = (text) => text.toLowerCase().replace(/[^a-z0-9\s]/g, '').split(/\s+/).filter(w => w.length > 3);
    const tokensStudent = new Set(clean(studentText));
    const tokensReference = new Set(clean(referenceText));
    if (tokensStudent.size === 0 || tokensReference.size === 0)
        return 0;
...
exports.calculatePlagiarismScore = calculatePlagiarismScore;
...
const gradeEssay = async (essayText, questionRubric, // Describes question prompt / grading criteria
totalMarks = 10) => {
    // Run Jaccard plagiarism check against reference database template
    const plagiarismPercent = (0, exports.calculatePlagiarismScore)(essayText, COPIED_WIKIPEDIA_SQL_ESSAY);
    if (OPENAI_API_KEY) {
        try {
...
const runLocalEssayGrading = (essay, rubric, totalMarks, plagiarismPercent) => {
    ...
    if (essay.includes('.') && essay.includes(','))
        grammar = Math.min(grammar + 1, 10);
    if (plagiarismPercent > 80)
        grammar = 9; // copied text usually has great grammar!
    // 2. Grade structure
    let structure = 4;
    if (essay.split('\n').length > 1)
        structure = 7; // multiple paragraphs
    if (essay.toLowerCase().includes('concl') || essay.toLowerCase().includes('summary'))
        structure = Math.min(structure + 2, 10);
